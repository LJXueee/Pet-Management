...
//	@RequestMapping(value = "/login",produces = "application/json;charset=UTF-8")
	...
	public ResultMap doRegist(User user) {
		System.out.println(user); //将内容打印到控制台
		User u = userService.getUserByPhoneAndName(user.getPhone(), null);
		if (u != null){
			return resultMap.success().message("该手机号已注册!");
		}
		try {
			user.setPassword(MD5.md5(user.getPassword())); //对用户密码进行加密（在utils包下）
			user.setCreateTime(new Date()); //自动填充user表的create_time项
			userService.save(user); //将注册的用户信息保存到表user中
			String[] ids = new String[1]; //创建一个String数组，并且数组长度为1，赋给ids
			ids[0] = user.getId()+""; //获取用户id
			// 普通用户
	...
	public String main(Model model) {
		...
		User user = (User) subject.getPrincipal();
		if (null != user) {
			model.addAttribute("user", user); //前端获取后端Controller中model里的值
		} else {
			return "login"; //跳转到用户登录界面
		}
