...
public class HealthController {
    ...
    public String applyListDoctor(Model model) {
        ...
        List<String> dsCount = new ArrayList<>(); //健康情况
        List<String> tsCount = new ArrayList<>(); //体温情况
        List<String> wsCount = new ArrayList<>(); //体重情况
        List<String> hsCount = new ArrayList<>(); //身高情况
        List<String> asCount = new ArrayList<>(); //饭量情况
        ...
        for(Pet p: rows){
            ...
            List<Diagnosis> dsRows = ds.getRows();//获取宠物的就诊记录条数
            int diagnosisStatus = 0;
            for (Diagnosis d: dsRows){
                diagnosisStatus += d.getStatus(); //宠物每次就诊的状况（1-正常；2-较重；3-严重；4-死亡）
            }
            int sw = diagnosisStatus / dsRows.size(); //计算宠物是否健康：sw=
            switch (sw){
    ...
    public Object tjDailyData(Long id){
        PetDaily daily = new PetDaily(); //PetDaily是pojo里定义的PetDaily实体类
        daily.setPetId(id);
        daily.setPage(1);
        daily.setLimit(99999);
        MMGridPageVoBean<PetDaily> ppp = (MMGridPageVoBean<PetDaily>)  petDailyService.getAllByLimit(daily);
